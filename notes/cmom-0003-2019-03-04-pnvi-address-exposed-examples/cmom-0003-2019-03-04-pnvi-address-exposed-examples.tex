...
\section{Refining the basic provenance model to support pointer
  construction via casts, representation Accesses, etc.}
...
\item \textbf{PNVI-ae (PNVI exposed-address)}: a variant of PNVI that
  ...
  \item \textbf{PNVI-aeudi (PNVI exposed-address
...
or
\end{itemize}
We write PNVI-* for PNVI-plain, PNVI-ae, and PNVI-aeudi.
The PNVI-plain and PVI semantics were described in the POPL 2019/N2311
paper.  PNVI-ae and PNVI-aeudi have emerged from discussions in the C
...
allocation.  For PNVI-ae and PNVI-aeudi, the allocation to \lstinline{y}
...
(here contrary to current GCC/ICC O2) while PVI
...
provenance. 
%
PNVI-ae and PNVI-aeudi need additional machinery, not yet developed in
...
\item Record symbolically in the semantics of integer values what such
  ...
  value $v$'', and allow reads of pointer values from
...
In PNVI-ae and PNVI-aeudi, it would not, and it is not clear that
there is any reasonable modification that would enable it. 
...
\end{minipage}
  ...
  \end{minipage}
  \end{center}
In PNVI-plain these would both work, while in PNVI-ae and PNVI-aeudi
...
\TODO{PNVI-ae and PNVI-aeudi considered up to here only}
