...
\title[PNVI-ae and PNVI-aeudi: detailed semantics]{C provenance semantics: detailed
semantics for the
...
and provenance-not-via-integer, address-exposed, user-disambiguation (PNVI-aeudi)
models, based on:
\begin{itemize}
...
Additional changes for PNVI-aeudi are \myu{highlighted}. 
...
{\color{red}This version does not support bytewise copy of a pointer
 ...
 machinery for PNVI-ae and PNVI-aeudi beyond the base PNVI (where
...
\section{The PNVI-aeudi, PNVI-ae, PNVI, and PVI semantics}
...
In PNVI-ae, PNVI, and PVI, a provenance $\pi$ is either
 $@i$ where $i$ is a storage-instance ID, or the \emph{empty}
  provenance $\provNone$. 
%
\myu{In PNVI-aeudi a provenance can also be a symbolic storage instance ID
...
\item \myt{for PNVI-ae and PNVI-aeudi, a taint flag $t\mathord{\in}\{\unexposed,\,\exposed\}$}.
\end{itemize}
\myu{In PNVI-aeudi, $A$ also maps all symbolic storage instance IDs $\iota$,
...
provenances are equal but both remain ambiguous, but that is not yet
spelt out.
}
...
index, the provenance of the result is the provenance of the bytes
(which will all have the same provenance). Otherwise, the $A$ part of the memory state is examined to find whether a live
storage instance exists with a footprint containing the pointer value that
is being constructed. If so, its storage instance ID
is used for the provenance of the pointer value, otherwise the empty
provenance is used.
\myt{In PNVI-ae and PNVI-aeudi, when constructing a pointer value, if $A$ has to
be examined then, following the relevant integer-to-pointer cast
semantics below, the storage instance must have been exposed,
otherwise  the memory
  action that was doing the $\abst()$ is UB.}
\myu{In PNVI-aeudi, if there are two such live storage instances, with
...
\myu{For PNVI-aeudi, the $\Kill$, $\load$, and $\store$  rules above must be adapted.  If $p=(\iota,a)$
...
\myu{For PNVI-aeudi, the rules above must be adapted. In the case where $p=(\iota,a)$
...
\myparagraph{Pointer subtraction} Pointers $p=(@i,a)$ and
      ...
      would have been flagged in earlier pointer arithmetic).
...
\myu{For PNVI-aeudi, because subtraction of pointers with different
provenance should be UB, if one of the two pointers has a symbolic storage
...
If both pointers are ambiguous, say mapped to $\{i_1,i_2\}$ and $\{i'_1,i'_2\}$,
then if those two sets share a single element which satisfies the other rule
preconditions, both symbolic storage instance IDs are resolved to
that.  If the sets have two pairs of elements that satisfy the other
conditions (which I believe can only happen if the addresses are
equal), then we should should arguably remember an identity of symbolic storage
instance IDs (this will need an extension to the state we have so
far, and additional modifications to the rules below beyond what is in
this draft). Otherwise UB. 
(This identity information is needed in princple because you might only know that they are the same, without
 knowing which.  Suppose \lstinline{p} and \lstinline{q} have been produced by separate casts
 ...
 the same provenance, but we still don't know which.  Alternatively,
 we could permit such subtractions and comparisons, leaving the
 ambiguity (of both pointers) unresolved.)}
\[
\ruleTwo
      {\diffPtr(\tau, p, p')=x}
      {label}
...
\myu{For PNVI-aeudi, this has to be adapted in much the same way as the
...
\ruleTwo
      ...
      {b = \mathit{op}(a, a') \qquad \mathit{op} \in \{\le, <, >, \ge\}}
      {A, M} {A, M}
\]
...
Pointers $p$ and $p'$ can always be compared with an equality operator
      ...
      false otherwise.
\myu{For PNVI-aeudi, because equality comparison is permitted (without
...
Then there must be a live storage instance for $i$, and the numeric
      ...
      \myu{For PNVI-aeudi, if $p$ is ambiguous (i.e., $p=(\iota,a)$ and
...
\myparagraph{Pointer member offset} Given a pointer $p$ at C type
        $\tau$,
        which is a struct or union type with a member $m$,
        if $p$ is some non-null $(\pi,a)$,
...
In PNVI, an integer-to-pointer cast of $0$ returns the null pointer.
...
integer value is exactly as in PNVI.  In addition, 
...
be \myt{\sout{properly}} within the
...
\myu{
\myparagraph{Casts (PNVI-aeudi)}
In PNVI-aeudi, a cast of a pointer value $p$ to an integer is just like
PNVI-ae.

In PNVI-aeudi, a cast of an integer value to a pointer,
as in the definition of $\repr$, 
can create a pointer with ambiguous provenance: 
