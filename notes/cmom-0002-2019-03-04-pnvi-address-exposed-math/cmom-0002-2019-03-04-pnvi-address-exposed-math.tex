...
It looks as if we also need to record a partial equivalence relation
...
equal), then we should remember an identity of symbolic storage
...
(This identity information is needed be because you might only know that they are the same, without
 knowing which.  Suppose $p$ and $q$ have been produced by separate casts
 from an integer which is ambiguously one-past one allocation and at
 the start of another. Then after $p-q$, we know they must have been
